...
import Link from "next/link"; // اضافه شدن Link برای دکمه ارتقا
...
export default function ProfilePage() {
  ...
  const handleImageUploadClick = () => {
    if (user?.subscription === "BASE") {
      setImageError(
        "Users with a BASE subscription cannot change their profile picture.",
      );
  ...
  return (
    <div className="flex flex-col gap-10 pb-10 transition-colors max-w-6xl mx-auto w-full relative">
        ...
        <section className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-sm animate-fade-in">
          ...
          >
            ...
            <div className="pt-2">
              <button
                type="submit"
                className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white font-bold rounded-full transition-colors w-full"
              >
                Save Changes
              </button>
            </div>
      ...
      {/* بخش جدید: کادر درخواست ارتقا به هنرمند (فقط برای شنونده‌ها) */}
      {user.role === "LISTENER" && (
