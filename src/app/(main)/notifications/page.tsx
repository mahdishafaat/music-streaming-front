...
export default function NotificationsPage() {
  ...
  const [notificationSetting, setNotificationSetting] = useState(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("app_notifications") || "all";
    }
    return "all";
  });
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      ...
      try {
        ...
        if (res.ok) {
          const data: BackendNotif[] = await res.json();

          let filteredNotifs = data;
          if (notificationSetting === "none") {
            filteredNotifs = [];
          } else if (notificationSetting === "mentions") {
            // 🌟 تغییر کلیدی: علاوه بر ارورها، نوتیف‌های SUCCESS (مثل تایید حساب) رو هم در دسته 'مهم' قرار بده
            filteredNotifs = data.filter(
              (n) =>
                n.type === "WARNING" ||
                n.type === "ERROR" ||
                n.type === "SUCCESS", // <--- این خط اضافه شد!
            );
          }

          const formattedData: Notification[] = filteredNotifs.map((n) => ({
            id: n.id.toString(),
            userId: user.id,

            // 🌟 تغییر کلیدی اینجاست: تبدیل LISTENER به USER یا نادیده گرفتن ارور
            targetRole: (user.role === "LISTENER" ? "USER" : user.role) as any,
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center py-32">
        <div className="animate-spin rounded-full h-10 w-10 border-t-4 border-b-4 border-blue-500 opacity-80"></div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl mx-auto w-full pb-10 transition-colors animate-fade-in">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center bg-gray-50 dark:bg-gray-800/30 rounded-2xl border border-dashed border-gray-200 dark:border-gray-700">
          ...
          >
            <path
              ...
              strokeWidth="1.5"
              d="M19.114 5.636a9 9 0 011.83 2.14M5.057 5.636A9 9 0 003.227 7.776M19.114 5.636l-14.057 12.728M19.114 5.636L5.057 18.364M3.227 7.776A9 9 0 005.057 18.364"
            />
          </svg>
              ...
              <div className="flex flex-col sm:flex-row justify-between gap-4 pl-6">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <h3
                      className={`font-bold text-lg ${notif.isRead ? "text-gray-800 dark:text-gray-200" : "text-gray-900 dark:text-white"}`}
                    >
                      {notif.title}
                    </h3>
                    {notif.type === "WARNING" && (
